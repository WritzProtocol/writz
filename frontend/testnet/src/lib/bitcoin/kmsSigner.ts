/**
 * KMS-backed protocol signer, plus `resolveProtocolSigner`'s testnet/signet
 * raw-WIF fallback.
 *
 * Canonical implementation lives in bitcoin-script/src/keys.ts. Both MUST
 * produce identical output for the same inputs - any change here must be
 * mirrored there and vice versa.
 *
 * Duplicated rather than imported because `frontend` has no working build
 * wiring to the local `bitcoin-script` workspace package: Next.js/Turbopack
 * fails to resolve a `file:`-linked package here ("Invalid symlink" during
 * `next build`), confirmed empirically while implementing this fix. This
 * mirrors the same constraint that already forced `address.ts` to duplicate
 * `bitcoin-script/src/script.ts` and `spend.ts` rather than import them.
 */
...
//
// Duplicated from bitcoin-script/src/keys.ts for the same reason KmsSigner
// itself is duplicated above - see this file's top comment.
 ...
 * contract expects, with `s` normalized to the low-S form Bitcoin policy
 * requires.
 *
