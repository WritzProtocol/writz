...
async function main() {
    ...
    // ── 1b. Generate a borrow_repay proof (a "borrow") ───────────────────────
    // No test vectors existed for this circuit before - see
    // `contracts/contracts/zk-verifier/src/test.rs`'s `verify_borrow_repay`
    // coverage, added alongside this generator update.
    const BR_COLLATERAL = 1_000_000n;        // satoshis
    const BR_OLD_DEBT    = 0n;               // fresh position
    const BR_SECRET      = 0xaabbccdd11223344n;
    const BR_NONCE       = 0x1122334455667788n;
    const BR_NEW_NONCE   = 0x9900aabb11223344n;
    const BR_PRICE       = 600_000_000_000n; // $60k, stroops per BTC
    const BR_MIN_RATIO   = 15_000n;          // 150% in bp
    const BR_BORROW      = 2_000_000_000n;   // $200 USDC - within the 150% ratio for this collateral

    const brOldCommitment = BigInt(poseidon.F.toString(
        poseidon([BR_COLLATERAL, BR_OLD_DEBT, BR_SECRET, BR_NONCE])
    ));
    const brTree = await buildSingleLeafTree(poseidon, brOldCommitment, 20);

    const borrowRepayInput = {
        collateral_satoshis:       BR_COLLATERAL.toString(),
        old_debt_stroops:          BR_OLD_DEBT.toString(),
        secret:                    BR_SECRET.toString(),
        nonce:                     BR_NONCE.toString(),
        new_nonce:                 BR_NEW_NONCE.toString(),
        path_elements:             brTree.pathElements.map(String),
        path_indices:              brTree.pathIndices.map(String),
        old_root:                  brTree.root.toString(),
        delta_stroops:             BR_BORROW.toString(),
        is_borrow:                 '1',
        btc_price_stroops_per_btc: BR_PRICE.toString(),
        min_ratio_bp:              BR_MIN_RATIO.toString(),
    };

    console.log('Generating borrow_repay proof…');
    ...
    const { proof: brProof, publicSignals: brSignals } =
        await snarkjs.groth16.fullProve(borrowRepayInput, brWasm, brZkey);
    if (!await snarkjs.groth16.verify(brVkey, brSignals, brProof))
        throw new Error('borrow_repay proof is invalid');
    console.log('Borrow/repay proof verified ✓');
    ...
    // Borrow/repay: 3 outputs (new_root, old_nullifier, new_commitment) then
    // 5 public inputs - order must match `borrow_repay_signals` in
    // contracts/contracts/commitment-tree/src/types.rs exactly.
    const borrowRepaySignalNames = [
        'new_root', 'old_nullifier', 'new_commitment',
        'old_root', 'delta_stroops', 'is_borrow', 'btc_price_stroops_per_btc', 'min_ratio_bp',
    ];
    ...
    const lines = [
        ...
        ...proofAndVkeyLines(depProof, depositVkey, depSignals, depositSignalNames, 'deposit',
            '// ── Deposit circuit ──────────────────────────────────────────────────────────'),
        '',
        ...proofAndVkeyLines(brProof, brVkey, brSignals, borrowRepaySignalNames, 'borrow_repay',
    ...
    console.log(`  Borrow/repay signals: ${brSignals.length}, IC: ${brVkey.IC.length}`);
