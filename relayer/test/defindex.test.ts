...
describe('POST /defindex/deposit', () => {
  test('200 with the unsigned deposit XDR, auto-investing on arrival', async () => {
    mockDepositToVault.mockResolvedValue({ xdr: 'AAAAAgAAAAA...', simulationResponse: {} });
    ...
    expect(mockDepositToVault).toHaveBeenCalledWith(
      VAULT_ID,
      { caller: VALID_ADDRESS, amounts: [10000000], invest: true },
      'testnet',
    );
  });

  test('400 when caller is missing, and the SDK is never called', async () => {
    const res = await request(app).post('/defindex/deposit').send({ amountStroops: '10000000' });
  ...
  test('400 when caller is malformed (wrong prefix)', async () => {
    const res = await request(app)
      .post('/defindex/deposit')
      .send({ caller: `C${VALID_ADDRESS.slice(1)}`, amountStroops: '10000000' });
  ...
  test('400 when amountStroops is missing, and the SDK is never called', async () => {
    const res = await request(app).post('/defindex/deposit').send({ caller: VALID_ADDRESS });
  ...
  test('502 with the ContractError variant name when the SDK rejects', async () => {
    mockDepositToVault.mockRejectedValue({
      error: 'ContractError',
      message: 'contract call failed',
  ...
  test('502 when the SDK resolves without an XDR to sign', async () => {
    mockDepositToVault.mockResolvedValue({ xdr: null, simulationResponse: {} });
