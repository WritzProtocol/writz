...
> **Status:** Draft post, not published. References to an upcoming SCF application below are aspirational - the application is a draft only, not submitted, with no short-term plan to submit.
...
We originally used `env.invoke_contract` with a `SpvResult` struct hand-copied into each caller crate, mirroring `bitcoin-spv`'s own return type field-for-field - worked because Soroban's Val encoding only cares about field names/types matching, not the Rust type's name or crate, but it meant three copies of the same struct with only a comment enforcing they stayed in sync. We've since pulled it into a small shared `spv-types` crate that all four contracts depend on, so there's one real type instead of three synchronized copies. Curious if others have hit the same thing, and whether `contractimport!` is the more idiomatic route for this versus a shared types crate - haven't compared the two closely.
