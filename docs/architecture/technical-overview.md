...
> **Superseded values notice:** This document predates several Phase 0/1 decisions and is kept for historical architecture context only. Where it conflicts with `docs/research/circom-circuit-design.md`, `docs/research/interest-rate-model.md`, `docs/research/oracle-design.md`, or the contract source (`contracts/contracts/private-lend/src/`), those are the sources of truth, not this file. Known stale values below have been corrected inline; treat any other specific number in this document as unverified until cross-checked against the docs above.
...
**Technology:** Circom circuit, verified via Groth16 inside Soroban using Protocol X-Ray's ZK verifier. (Corrected - this section originally specified Noir; the adopted circuit language is Circom, see `docs/research/circom-circuit-design.md`.)
...
if utilization <= optimal_utilization (75%):
    borrow_rate = base_rate + (utilization / optimal) × slope1
else:
    borrow_rate = base_rate + slope1 + ((utilization - optimal) / (1 - optimal)) × slope2
```
(Corrected - this section originally showed 80% optimal utilization; the finalized value is 75%, see `docs/research/interest-rate-model.md`.)
...
| ZK proofs | Circom circuits + Groth16 via Protocol X-Ray |
