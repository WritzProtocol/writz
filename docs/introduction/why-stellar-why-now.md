...
*(Updated 2026-08-04 - see `docs/research/market-landscape.md` for the full competitive picture.)* The BTCfi category on Stellar is not empty anymore: Solv Protocol runs an audited, custodial wrapped-BTC deployment with real trading volume, and Templar Protocol has a live, audited Soroban vault plus native-BTC-without-wrapping lending tech already proven on NEAR. Neither offers what Writz does: **trustless, SPV-verified native BTC collateral combined with ZK-private positions.** That specific combination is still open on Stellar today.

The window for a from-scratch entrant to replicate that combination is 12–18 months wide - building a Bitcoin SPV client on Soroban, running a Groth16 trusted setup ceremony, getting audited, and launching mainnet is not a weekend project. But for Templar specifically, that window is materially shorter: their Stellar infrastructure and native-BTC tech already exist separately, so connecting them is integration work, not a from-scratch build. Testnet-to-mainnet speed is a real competitive variable against Templar, not just a generic risk. A well-funded team starting today would arrive in early-to-mid 2028 - by which time Writz will have:
...
Protocol X-Ray shipped in January 2026. The compute budget was confirmed feasible. The Rust SPV library exists. Native USDC is live. No protocol on Stellar combines trustless native BTC collateral with ZK-private positions yet - but Templar Protocol already has the adjacent infrastructure live and in active development. The window is open right now, and it will not stay open forever.
